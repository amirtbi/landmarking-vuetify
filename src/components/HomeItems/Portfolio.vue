  ...
  <v-container class="my-15">
    ...
    <v-row class="mt-4" justify="center">
      ...
      <v-col cols="12" lg="12">
        <div class="text-center" :class="screenChange">
          <v-chip
            v-for="item in chips"
            :key="item.title"
            @click="switchPortfolio(item.title)"
            :color="activePortfolio === item.title ? 'green-darken-4' : ''"
            text-color="white"
            cursor-pointer
            :class="{
              'on-hover': ishovering,
      ...
      <v-col
        ...
        cols="12"
        sm="6"
        class="portfolio-image__container"
        v-for="item in portfolios[selectedCategory]"
        :key="item.title"
      >
        <v-card>
          <v-img
